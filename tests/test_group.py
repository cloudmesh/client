...
class Test_group:
    ...
    def test_001(self):
        ...
        id = str(Group.get(name="groupA", cloud="kilo"))
        print id
        assert "test-001" in id
        return

    def test_002(self):
        """testing cm group add groupA --cloud=kilo --id=test-002 --type=vm"""
        HEADING()
        self.sh("cm group add groupA --cloud=kilo --id=test-002 --type=vm")
        group = Group.get(name="groupA", cloud="kilo")
        assert group["cloud"] == "kilo"
    ...
    def test_007(self):
        """testing cm group list --cloud=kilo --format table"""
        HEADING()
        banner("cm group list --cloud=kilo --format=table")

        result = run("cm group list --cloud=kilo --format=table")
        assert "groupA" in result
        return
    ...
    def test_008(self):
        ...
        result1 = run("cm default cloud=kilo")
        ...
        result3 = run("cm group list --cloud=kilo groupX")
    ...
    def test_009(self):
        """testing cm group remove --cloud=kilo --name=groupA --id=test-002"""
        HEADING()
        banner("cm group remove --cloud=kilo --name=groupA --id=test-002")

        result1 = run("cm group remove --cloud=kilo --name=groupA --id="
                      "test-002")
        print(result1)
    ...
    def test_010(self):
        """testing cm group delete groupA --cloud=kilo"""
        HEADING()
        """
        banner("cm group delete groupA --cloud=kilo")
        result = run("cm group delete groupA --cloud=kilo")
        print(result)
        assert "ok." in result
        """

        banner("cm group delete groupB --cloud=kilo")
        result = run("cm group delete groupB --cloud=kilo")
        assert "ok." in result

        banner("cm group delete groupC --cloud=kilo")
        result = run("cm group delete groupC --cloud=kilo")
        assert "ok." in result
